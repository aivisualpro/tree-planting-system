...
const generatePdf = async (type: string, params: Record<string, string>) => {
  exporting.value = type
  try {
...
<template>
  <div class="p-8 max-w-7xl mx-auto space-y-10">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-4xl font-black text-green-900 tracking-tight">Reporting Center</h1>
        <p class="text-muted-foreground text-lg mt-2">Export data and generate compliance-ready reports.</p>
      </div>
    ...
    <div class="grid md:grid-cols-2 gap-8">
      <Card v-for="section in reportTypes" :key="section.title" class="overflow-hidden border-2 hover:border-green-600/20 transition-all duration-300">
        <CardHeader class="bg-muted/30 pb-8">
          <CardTitle class="text-2xl">{{ section.title }}</CardTitle>
          <CardDescription class="text-base">{{ section.description }}</CardDescription>
        </CardHeader>
    ...
    <div class="bg-blue-50 border border-blue-100 rounded-2xl p-8 flex items-center gap-8">
       ...
       <div>
          <h3 class="text-xl font-bold text-blue-900 mb-1">Scheduled Reports</h3>
          ...
          <Button variant="link" class="p-0 h-auto text-blue-600 font-bold mt-4" as-child>
            <NuxtLink to="/settings/reports">Manage Subscriptions &rarr;</NuxtLink>
          </Button>
       </div>
    </div>
  </div>
